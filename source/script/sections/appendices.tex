...
\subsection{Gauss-Seidel iteration | better splitting method\skipthis}
\label{app:gauss_seidel}
...
\begin{figure}[H]
    ...
    \caption{Steepest descent for solving a linear system $\mat{A}\vec{x} = \vec{b}$ with positive definite matrix $\mat{A}$.}
...
\begin{figure}[H]
    ...
    \caption{Conjugate gradient method for solving a linear system $\mat{A}\vec{x} = \vec{b}$ with positive definite matrix $\mat{A}$.}
...
\begin{itemize}
    ...
    \item ...
\end{itemize}
\note{The best basis for the Krylov subspace $\mathcal{K}_r$ is orthonormal and can be computed using Arnoldi's method (essentially Gram-Schmidt).
...
in the implicit differentiation is a computational overhead compared to the backward mode automatic
...
evaluation points and weights. One such method is called Gauss-Legendre quadrature, where the evaluation
points are the roots of \textit{Legendre polynomials}.

\subsubsection{Legendre Polynomials}

The Legendre Polynomials are an orthogonal system of polynomials,
$P_n \in \Pi_n$, on the interval $[-1,1]$ with 
...
\begin{equation}
    P_0(\xi)=1, \quad P_1(\xi)=\xi, \quad P_2(\xi) = \frac{1}{2}\left(3 \xi^2-1\right)
\end{equation}
...
\begin{equation}
    \begin{gathered}
        \int_{-1}^1 f(\xi) d \xi \approx \sum_{q=0}^n f\left(\xi_q^{1 D}\right) w_q^{1 D} \\
        \text{roots } \xi_q^{1 D} \text{ of } P_{n+1}(\xi), \text{ weights } w_q^{1 D}=\frac{2}{\left(1-\left( \xi_{q}^{1 D} \right)^2\right)\left(P_{n+1}^{\prime}\left(\xi_{q}^{1 D}\right)\right)^{2}}
    \end{gathered}
\end{equation}
for $f: [-1,1] \rightarrow \mathbb{R}$ is exact for polynomials of degree $2n+1$.

\subsubsection{Proof: Gauss-Legendre quadrature with $n+1$ evaluation points is exact for polynomials of degree $2n+1$}

Let $f = P(x)$ have degree $\leq 2n+1$. Then we can write
...
\begin{equation}
    \begin{aligned}
        ...
        &=\int_{-1}^{1} R d x \\
        &=\underbrace{\sum_{q=0}^{n} R\left(\xi_{q}^{1 D}\right) w_{q}^{1 D}}_{\text{exact as } R \text{ is a polynomial of degree } \leq n} \\
        &=\sum_{q=0}^{n} \underbrace{\left(\underbrace{Q(\xi_{q}^{1 D}) \cdot P_{n+1}(\xi_{q}^{1 D})}_{=0, \text{as } \xi_{q}^{1 D} \text{ roots of } P_{n+1}} + R\left(\xi_{q}^{1 D}\right)\right)}_{=P(\xi_{q}^{1 D})} w_{q}^{1 D} \\
        &= \sum_{q=0}^{n} P(\xi_{q}^{1 D}) w_{q}^{1 D}
    \end{aligned}
\end{equation}
...
\begin{figure}[!htb]
 \centering
 \includesvg[width=0.8\textwidth]{figures/distance_hist.svg}\hfill
...
Note that for the inverse transform we need the inverse CDF, so we need to integrate the PDF. So choosing $f(x)$ itself

...
\begin{equation}
    R_n(x) = \frac{f^{(n+1)}(\eta)}{(n+1)!}(x-x_0)^{n+1} \in \mathcal{O}(|x-x_0|^{n+1}), \quad \eta \in [x_0, x]
\end{equation}
...
\begin{itemize}
    \item little $o$: $f \in o(g) \iff \lim_{x \to a} \left| \frac{f(x)}{g(x)} \right| = 0$ ($f$ grows slower than $g$)
...
\begin{equation}
    \begin{gathered}
        \left. \begin{matrix}
            f_{i+1} = f_{i} + \Delta x \, f'_i + \frac{1}{2} f''_i \Delta x^2 + \frac{1}{6} f'''_i \Delta x^3 + \mathcal{O}(\Delta x^4) \\
            f_{i-1} = f_{i} - \Delta x \, f'_i + \frac{1}{2} f''_i \Delta x^2 - \frac{1}{6} f'''_i \Delta x^3 + \mathcal{O}(\Delta x^4)
        \end{matrix} \right\} \\
        \rightarrow f''_i = \frac{f_{i+1} - 2 f_{i}  + f_{i-1}}{\Delta x^2} + \mathcal{O}(\Delta x^2)
    \end{gathered}
\end{equation}
...
\begin{equation}
    n_0 := 1, \quad n_j := (x-x_0) \cdot \dots \cdot (x - x_{j-1}), j = 1,...,m, \quad \text{short:} \, n_j(x) = \prod_{i = 0}^{j - 1} (x-x_i)
\end{equation}

...
% write about why binary
% use simpler model to describe basic floating point problems

We want to operate on numbers on a computer where storage is finite, and 
fixed-size representations in terms of discrete states are sought for efficient operations. However,
the natural numbers are a countably infinite set and the real numbers 
are an uncountably infinite set. There is simply no finite-size encoding
which could exactly represent every number, let's say every real between 
$0$ and $1$ - a fundamental error source of numerics. These restrictions
result in caveats.

In C, \mintinline{C}{int x = 100 * 200 * 300 * 400;} will surprisingly 
yield $-1894967296$ (for a 32-bit integer representation) (\textit{overflow}), 
\mintinline{C}{float f1 = (2.3 + 1e20) - 1e20;} yields $0.0$ (\textit{round-off-error}) but
\mintinline{C}{float f2 = 2.3 + (1e20 - 1e20);} correctly gives $2.3$.

We want to understand and mitigate such caveats of arithmetic on computers, 
where we want quick calculations while also
using as little storage as possible.

Further details can be found in \cite{higham02} and \cite{bryant2011computer}.

\subsection{Integer Arithmetic}

In C, integers ($\in \mathbb{Z}$) are stored as fixed-size bit-sequences. While for us humans
with commonly $10$ fingers a base-10 number system is natural, two-state systems are physically
easier to construct and reliably operate on (e.g. a transistor conducts or not) (also see \cite[Sec. 2.7]{higham02}). Therefore,
we are dealing with binary (base-2) representations throughout this chapter.

The respective size of the bit-sequence dictates a range. C provides both unsigned and signed 
integer types, with negative numbers in the signed case represented using \textit{two's-complements}.
...
(illustrated in Fig. \ref{fig:unchar}) and the range is $0,\dots,2^\omega - 1$. In case of overflow in operations, the overflow is truncated
in the most significant bits (see Fig. \ref{fig:uncharadd}). As $B2U_{\omega}[x_{\omega-1},x_{\omega-2},\dots,x_0] \mod 2^k = 
...
$x_0$ to $x_{\omega-2}$ with correct sign-switch by carry-on, no special handling is necessary (see Fig. \ref{fig:tcompl}). The range is $-2^{\omega-1} \dots 2^{\omega-1} - 1$.
}
...
\end{enumerate}
These rules intuitively follow from the constraint, that bitwise addition with 
carry-on of $-u$ and $u$ should result in an all-zero bitvector. Adding a bitvector 
to its inverted self results in an all-1 bitvector, adding one more then results in 
all zeros, as the last carry-on is discarded.
...
% Bytes of e.g. a multi-byte integer can be stored from most significant byte to least significant byte (big endian) or vice versa (little endian), see Fig. \ref{fig:endian}.
...
\textit{Example:} In \mintinline{C}{char c = 100 * 4; // range -128 to 127} the result is $-112$ as $400$ in bits is $0001 1001 0000$ where a cut-off to one byte means $1001 0000$ so $-2^7+2^4 = -128 + 16 = -112$.\par
}
...
%         \textcolor{green1}{implicit type conversion up to int can be helpful} & \textcolor{red1}{mind it's only up to int} & \textcolor{green1}{explicit type conversion} \\
%         \hline
...
\end{equation}
\yellowbox{\textbf{Historic example:} In the first Gulf War (more specifically on 25th February 1991), a Patriot missile defense battery failed to intercept an incoming Iraqi Scud missile, because it used an internal clock counting up in tenths of a second represented by a 23-bit sequence. Future missile positions are predicted by extrapolating from past position with constant velocity. The Patriot system mixed both this inaccurate internal clock and a more accurate one, leading to the failed interception and 28 deaths among soldiers.}

\subsubsection{An intuition for limited precision in fixed-size scientific notation}
Consider we want to design a fixed-size representation of real numbers, let's say we can store six decimal digits and two signs.
We choose our format to be

\begin{equation}
    \pm d_1.d_2 d_3 d_4 \cdot 10^{\pm d_5 d_6}, \quad \text{e.g.} \, +9.821 \cdot 10^{-12}
\end{equation}

\greybox{\textbf{Task to the reader:} Think about the range of this representation and the number closest to $0$ we can represent.}
...
However, consider we have stored $1$ then the smallest increment 
we can resolve in the mantissa is $0.001$ - \textbf{our precision is limited and relative!}
...
\begin{itemize}
    ...
    allows to approach zero with gradually decreasing precision (and even spacing) (smaller numbers occupy fewer digits as of the leading zeros) and reduces the risk of underflow.
...
A specific example of a floating point representation is given in Fig. \ref{fig:523float}.
...
\begin{figure}[!htb]
    \centering
    \includesvg[width=1.0\textwidth]{figures/523float.svg}\hfill
...
\bluebox{\textbf{Finite floating point precision is one of the fundamental error sources in numerics.}}

Given $x$ is in the range of representable floating point numbers, then the floating point representation

\begin{equation}
    fl(x) = x \cdot (1+\delta), \quad |\delta| \leq \epsilon_{\text{mach}}
\end{equation}

\citep[Theorem 2.2]{higham02}. The modified formulation

\begin{equation}
    fl(x) = \frac{x}{1+\delta}, \quad |\delta| \leq \epsilon_{\text{mach}}
\end{equation}

also holds \citep[Theorem 2.2]{higham02}. For multiple floating point operations
the following result can be helpful \citep[Lemma 3.1]{higham02}: If $|\delta_i| \leq \epsilon_{\text{mach}}$
and $\rho_i \in \{+1,-1 \}$ for $i = 1,...,l$ and $l \epsilon_{\text{mach}} < 1$
then

\begin{equation}
    \label{eq:mult_float_bound}
    \prod_{i=1}^{l} (1+\delta_i)^{\rho_i} = 1 + \theta_l, \quad |\theta_l| \leq \frac{l \epsilon_{\text{mach}}}{1 - l \epsilon_{\text{mach}}} =: \gamma_l
\end{equation}
...
\begin{equation}
    \text{relative error } \frac{|x-\hat{x}|}{|x|} \leq \epsilon_{\text{mach}}, \quad \text{number } x, \text{ number on machine } \hat{x}
\end{equation}
...
\begin{itemize}
    ...
    \item \textcolor{red1}{\textbf{Associativity is not guaranteed:}} $(a+b)+c \explain{\ne}{i.A.} a+(b+c)$, e.g. $(2.3 + 1e20) - 1e20$ yields $0.0$ but $2.3 + (1e20 - 1e20)$ yields $2.3$
...
\begin{itemize}
    ...
    \item \textcolor{green1}{\textbf{Avoid overflow to inf and nan:}} E.g. in \mintinline{C}{float x = 1e20; float y = x * x; float z = y / x} $z$ will be inf. 
\end{itemize}
...
\end{equation}
A comparison of both versions in python can be found in Fig. \ref{fig:cancellation_ex}, at some point (roughly below $10^{-8})$, $\cos x$ is too close to $1$ to be resolved 
by the mantissa (consider $\cos x = 1 - \frac{x^2}{2} + ...$, for $x \sim 10^{-8}$, the $x^2$ term falls below what machine precision can resolve), so the total result is $0$, above that the magnified error is visible.
...
of \textbf{backward stability}. A weaker formulation is the mixed forward-backward error
\begin{equation}
...
\end{equation}
so ill-conditioned problems can have large forward errors. Note that the condition number is inherent to the problem
not the numerical implementation.

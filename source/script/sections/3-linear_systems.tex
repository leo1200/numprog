...
Solving linear systems $\mat{A}\vec{x} = \vec{b}$ (invertible $\mat{A} \in \mathbb{R}^{n \times n}, \vec{x} \in \mathbb{R}^{n}, \vec{b} \in \mathbb{R}^{n}$) is important throughout scientific computing and
at the core of many numerical algorithms.
...
\begin{equation}
    \partial_{\vec{\beta}} ||\vec{y}^{\text{data}} - \mat{X} \vec{\beta}||_2^2 = -2 \mat{X}^T(\vec{Y}-\mat{X} \vec{\beta}) \underbracket{=}_{!} 0
\end{equation}

and written as a linear system

\begin{equation}
   \mat{X}^T \mat{X} \vec{\beta} = \mat{X}^T \vec{Y}
\end{equation}

\subsection{Gaussian Elimination}

Go through practical example of Gaussian elimination, formulate the general principle.

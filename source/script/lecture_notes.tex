...
% not needed with v3+ \usepackage[outputdir=loux]{minted}
\usepackage{minted}
...
% Custom float and list for code snippets without tocbasic
\newfloat{codebox}{tbp}{lor}
\floatname{codebox}{Code-Snippet}
\newcommand{\listofcodeboxes}{%
  \addcontentsline{toc}{section}{List of Code-Snippets}%
  \listof{codebox}{List of Code-Snippets}%
}
...
% Usage: \rowOp{sourceRow}{targetRow}{operationText}
\newcommand{\rowOp}[3]{
    \begin{tikzpicture}[remember picture, overlay]
        % 1. Circle the diagonal element (standard thickness)
        \draw (#1-#1) circle (1.8ex);
        
        % 2. Define key points for the arrow
        % A = point to the right of the source row
        % B = point to the right of the target row
        \coordinate (A) at ([xshift=0.7cm]#1-last.east);
        \coordinate (B) at ([xshift=0.7cm]#2-last.east);
        
        % 3. Draw the hook arrow
        % We move 0.4cm further right from A, then drop down to B
        \draw[thick, ->, >=Stealth] (A) -- ++(0.4,0) coordinate (verticalBar) |- (B);
        
        % 4. Place the text vertically centered on the vertical segment
        % We find the midpoint between A and B vertically, at the x-position of the vertical bar
        \node[right, inner sep=4pt] at ($(verticalBar)!0.5!(verticalBar |- B)$) {#3};
    \end{tikzpicture}
}
...
\begin{abstract}
  ...
  numerical solutions to linear systems, numerical derivatives, optimization
